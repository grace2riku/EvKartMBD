...
Arduinoに合体できるようにオスのピンヘッダ1列が2個あります。
また、インバーター基板と合体できるようにオスの50ピン2列のピンヘッダが2個あります。
@<img>{gw+driver_pcb}はインバーター基板と治具基板を合体させた時の写真です。
//image[gw+driver_pcb][インバーター基板+治具基板の合体写真]{
//}

@<img>{gw+driver+arduino}はインバーター基板+治具基板にArduinoを合体させた時の写真です。
//image[gw+driver+arduino][インバーター基板+治具基板+Arduinoの合体写真]{
//}
 ...
 * 無償でモデルベース開発に対応しているのがArduinoだったから。他にラズベリーパイも対応しているがEVカートの制御内容・ボリュームを考えるとArduinoの性能がベストと考えた。
...
モーター制御に使う信号は@<table>{arduino_mega_pinout_table}のArduino MEGAの各ピンに割り当てています。
...
Simulinkはブロックと呼ばれる部品を配置・設定し、ブロック同士をつなげていきモデルをつくります。
...
また、Arduinoのペリフェラルを制御する各種ブロックを提供します。
たとえば、GPIO入力・出力、AD変換、PWMなどです。Arduinoのペリフェラルを制御する場合はこのブロックを使用します。
