...
下記リンクのカート本体とモーター・インバーターキットを組み合わせることで走行可能なカートを製作できます。
...
//}
ブラシレスモーターに3相の信号線、ホールセンサーが接続されています。

このブラシレスモーターは12極18スロットのアウタ・ロータタイプです。@<br>{}
「12極」とは磁石が合計12個あることです。@<img>{motor_overall}の黒い部分が磁石です。N極・S極が交互に並んでいます。@<br>{}
「18スロット」とはエナメル線が巻かれたコイルが18個あることです。@<img>{motor_overall}の金色部分がエナメル線を巻いたコイルです。
このコイルにエナメル線を手巻きしモーターをつくりあげていきます。@<br>{}
@<img>{motor_overall}は私が手巻きしたモーターです。巻線の方式は「6直」で、φ1.0mmのエナメル線を1スロットあたり18回巻いています。
この巻線仕様だと他の巻線仕様（2直3並、3直2並）と比べ次の特性となります。

 * 最高速度：2直3並 > 3直2並 > 6直
 * トルク：2直3並 < 3直2並 < 6直

巻線の方式は@<img>{6serial}、@<img>{2serial_3parallel}、@<img>{2serial_3parallel}を参照ください。
各相6個ずつU相・V相・W相の合計18スロットを手巻きします。
//image[6serial][6直][scale=0.5]{
//}
//image[2serial_3parallel][2直3並][scale=0.5]{
//}
//image[3serial_2parallel][3直2並][scale=0.5]{
//}

巻線6直のモーターは「速度は遅いがトルクは大きい」特性となります。勾配のあるコースでは有利ですが、平坦が多いコースでは不利です。
以前回転数計で計測したときはスロットルを最大まで開けたときに1000rpmくらいでした。

「アウタ・ロータ」ですが磁石の付いた「外側のロータ」が回転することからそう呼ばれています。
...
@<img>{HallandPWMControl}の制御を行うとモーターは物理的に「60度」回転します。
別の言い方をすると@<img>{HallandPWMControl}の制御を6回繰り返すとモーターは360度回転します。
これは@<hd>{ブラシレスモーター}、@<hd>{ホールセンサー}で説明したモータの極数は12極、N・S極検出で出力信号レベルが切り替わることから説明できます。
このブラシレスモーターが物理的に360度回転するとき、極数は12極のためホールセンサーのエッジは12回変化します。
@<img>{HallandPWMControl}は1つのホールセンサーに注目すると2回エッジが変化しています。

 * 12 / 2 = 6 → @<img>{HallandPWMControl}は物理的に360度回転するうち1/6の期間を示している
 * 360 / 6 = 60 → @<img>{HallandPWMControl}は物理的に60度回転する期間を示している

よって、@<img>{HallandPWMControl}は360度回転するうちの60度分の期間を示していることがわかります。
モーターが物理的にどのくらい回転するかは極数でわかります。
...
ルネサス エレクトロニクス株式会社のリンク@<href>{https://www.renesas.com/jp/ja/support/technical-resources/engineer-school/brushless-dc-motor-02-inverter-pmw.html}は
ブラシレスモーターの制御がアニメーションでわかりやすく説明されているのでお薦めです。
本書と同じブラシレスモーター制御はページ中ほどの図3です。
